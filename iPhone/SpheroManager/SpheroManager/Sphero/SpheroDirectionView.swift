...
//    TODO: add lunching event (shock detection)
...
//            currentAccData.removeAll()
        ...
        func predictShape() {
            ...
            do {
                if let prediction = try neuralNet?.update(inputs: normalizedAccFloat),
    ...
    private func stopPredictAnyTime() {
        ...
        neuralNet = FFNN(inputs: currentAccData.count, hidden: 20, outputs: 3, learningRate: 0.3, momentum: 0.2, weights: nil, activationFunction: .Sigmoid, errorFunction:.crossEntropy(average: false))
        inputQuantity = Double(currentAccData.count)
        ...
        VStack {
            HStack {
                ...
                Button(action: { startRecording() }) {
                                    Text("Start Recording").padding().background(Color.blue).foregroundColor(.white).cornerRadius(10)
                }
            }
                                
                    if (isRecording) {
                        Button(action: { stopRecording() }) {
                                    Text("Stop Recording").padding().background(Color.red).foregroundColor(.white).cornerRadius(10)
                                }
                                }
                        if (isStopped) {
                            Button(action: { predictShape() }) {
                                    Text("Predict Shape").padding().background(Color.green).foregroundColor(.white).cornerRadius(10)
                                }
                                }
