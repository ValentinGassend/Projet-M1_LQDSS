...
extension WebSocketClient: WebSocketConnectionDelegate {
    ...
    func sendMessage(
        ...
        //                        print("Message sent to \(originMessage): \(formattedMessage)")
        //                    } else {
        //                        print("Route \(originMessage) not found!")
        //                }
    }
    ...
    private func routeMessage(_ message: String, for route: String) {
        ...
        for target in parsedMessage.routeTargets {
            print("Processing message for target: \(target)")
            
            // Traitement basé sur le composant
            switch parsedMessage.component {
            case "rfid":
                handleRFIDMessage(parsedMessage, target: target)
            case "relay1", "relay2", "relay3", "relay4":
                handleRelayMessage(parsedMessage, target: target)
            case "sphero1", "sphero2", "sphero3", "sphero4":
                handleSpheroMessage(parsedMessage, target: target)
            default:
                print("Unknown component: \(parsedMessage.component) for target \(target)")
            }
        }
    }
    
    // Exemple de traitement pour RFID
    private func handleRFIDMessage(_ message: ParsedMessage, target: String) {
        print("RFID message received for \(target): \(message.data)")
        if target.contains("maze") {
    ...
    private func handleSpheroMessage(_ message: ParsedMessage, target: String) {
        print("Sphero message for \(message.component) on \(target): \(message.data)")
    }
    // Exemple de traitement pour Relay
    private func handleRelayMessage(_ message: ParsedMessage, target: String) {
        print("Relay message for \(message.component) on \(target): \(message.data)")
    }
